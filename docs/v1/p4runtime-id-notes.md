...
| `digest_id` | `DigestEntry` | `Digest` | Used to configure a `Digest` object as a whole, not to deal with digest messages. |
| `digest_id` | `DigestList` | `Digest` | These messages are sent by a server to a client, to report a list of digest messages generated by a P4 device. |
| `digest_id` | `DigestListAck` | `Digest` | These mssages are sent by a client to acknowledge an earlier `DigestList` message that it received. |
