...
As per section [X](#sec-default-valued-fields), default values are invalid for all scalar P4 object IDs.
This includes `device_id`, so the `0` ID may not be assigned to a device.

The "default device_id" corresponds to `0`. As mentioned [previously](),
