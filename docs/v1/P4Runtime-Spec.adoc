...
`SizeSemantics` message, used for resource accounting. The `BucketSelectionMode`
 enum consists of `DEFAULT_MODE_DETERMINED_BY_ACTION_SELECTOR`, `HASH`, and `RANDOM`.
The `SizeSemantics` enum consists of
`DEFAULT_SIZE_DETERMINED_BY_ACTION_SELECTOR`, `SUM_OF_WEIGHTS`, and
`SUM_OF_MEMBERS`, which dictates the hashing mode to use for the packet. 
