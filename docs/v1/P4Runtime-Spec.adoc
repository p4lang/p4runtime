...
* `action_refs`, a repeated `ActionRef` field representing the set of possible
  ...
  specified in the same P4Info message and it includes the following fields:
    ...
    ** `scope`, an enum value which can take one of four values:
       `TABLE_AND_DEFAULT`, `TABLE_ONLY`, `GLOBAL_ONLY` and `DEFAULT_ONLY`. The
       `scope` of the action is determined by the use of the P4 standard
       annotations `@tableonly`, `@globalonly`, and `@defaultonly`
       cite:[P4ActionAnnotations]. `TABLE_ONLY` (`@tableonly` annotation) means
       that the action can only appear within the table, and never as the default
       action. `GLOBAL_ONLY` (`@globalonly` annotation) means that the action can
       can only appear within the table, but only used in the `global_action`
       parameter in `ActionProfileActionSet`. `DEFAULT_ONLY`
       ...
       used both within the table and as the default action, but not as a global
       action.
...
`ActionProfileAction` messages, an `Action` for the global action, a
`ActionSelectionMode` enum, and a `SizeSemantics` enum. The `ActionSelectionMode`
and `SizeSemantics` enums override the existing action selector settings.
...
The `global_action` is a native global parameter mechanism that behaves as a
"base" action that is logically composed with the selected member action.
Semantically, this functions as a pre-action to the existing member-specific
logic, effectively providing a shared context needed for the pipeline to
process any selected `ActionProfileAction`. Here, the global action that sets
the route metadata would occur first and then the nexthop gets set for each
`ActionProfileAction`.
