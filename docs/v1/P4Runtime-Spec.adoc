...
* `action_refs`, a repeated `ActionRef` field representing the set of possible
  ...
  specified in the same P4Info message and it includes the following fields:
    ...
    ** `scope`, an enum value which can take one of four values:
       ...
       `@pergrouponly` is currently not a P4 standard annotation. `TABLE_ONLY`
       (`@tableonly` annotation) means that the action can only appear within
       the table, and never as the default action. `PER_GROUP_ONLY`
       (`@pergrouponly` annotation) means that the action can only be used in as
       a `per_group_action` in an `ActionProfileActionSet`. `DEFAULT_ONLY`
...
The `per_group_action` behaves as a "base" action that is logically composed with
