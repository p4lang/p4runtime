...
* `action_refs`, a repeated `ActionRef` field representing the set of possible
  ...
  specified in the same P4Info message and it includes the following fields:
    ...
    ** `scope`, an enum value which can take one of four values:
       ...
       that the action can only appear within the table, and never as the
       default action. `GLOBAL_ONLY` (`@globalonly` annotation) means that
       the action can only appear within the table, but only used in the
       `global_action` parameter in `ActionProfileActionSet`. `DEFAULT_ONLY`
...
The `global_action` is a native global metadata setting mechanism for
`ActionProfileActionSet`s. This behaves as a "base" action that is logically
composed with the selected member action. Semantically, this functions acts as
a pre-action to the existing member-specific logic, effectively providing a
shared context needed for the pipeline to process any selected
`ActionProfileAction`. For example, the global action that sets the route
metadata would occur first and then the nexthop is set for each
`ActionProfileAction`. Note that this functionality can only be used with
the one shot method.
