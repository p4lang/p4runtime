...
action selector, ensuring backward compatibility.

* `HASH`: selects an action based on the hash mode from the action selector.

* `RANDOM`: selects an action at random.

Each `SizeSemantics` enum can take on the following values:
* `DEFAULT_SIZE_DETERMINED_BY_ACTION_SELECTOR`: uses the size specified by the
action selector, ensuring backward compatibility.
...
DEFAULT_SIZE_DETERMINED_BY_ACTION_SELECTOR values for BucketSelectionMode and
SizeSemantics respectively. Servers must also support the specific value
combination that corresponds to that default. Otherwise, if the target does not
support per-group hash settings, the server should return UNIMPLEMENTED. If the
target supports per-group hash settings, but not the specific combination given,
the server should return INVALID_ARGUMENT.
