...
* `DEFAULT_MODE_DETERMINED_BY_ACTION_SELECTOR`: uses the action specified by the
  action selector, ensuring backward compatibility. In a SAI-based switch, this
  would correspond to setting `SAI_NEXT_HOP_GROUP_ATTR_HASH_ALGORITHM` to
  `SAI_HASH_ALGORITHM_NONE`, using the default action selector mode.

* `HASH`: selects an action based on the hash mode from the action selector. In
  a SAI-based switch, this would correspond to setting
  `SAI_NEXT_HOP_GROUP_ATTR_HASH_ALGORITHM` to `SAI_HASH_ALGORITHM_CRC`,
  `SAI_HASH_ALGORITHM_XOR`, `SAI_HASH_ALGORITHM_CRC_32LO`,
  `SAI_HASH_ALGORITHM_CRC_32HI`, `SAI_HASH_ALGORITHM_CRC_CCITT`, or
  `SAI_HASH_ALGORITHM_CRC_XOR`.

* `RANDOM`: selects an action at random. In a SAI-based switch, this would
  correspond to setting `SAI_NEXT_HOP_GROUP_ATTR_HASH_ALGORITHM` to
...
support per-group hash settings, the server should return `UNIMPLEMENTED`. If
the target supports per-group hash settings, but not the specific combination
given, the server should return `INVALID_ARGUMENT`.
